...
\item[(iv)] decimos que \emph{el problema tiene solución} si y solamente si el problema es acotado inferiormente y $\exists\xo\in\Rn$ tal que $f(\x)=\po$. Tal $\xo$ recibe el nombre de \emph{punto óptimo} o \emph{solución del problema}.
\end{itemize}
\end{definition}
...
\begin{itemize}
\item[(i)] Si $S=\{\x\in\R: h_{j}(\x)=0\ \ \forall j=\Sq\}$ donde $h_{j}:D\subseteq\Rn\rightarrow\R\ \ \forall j = \Sq$, escribimos
\begin{equation*}
\begin{aligned}
& \underset{\x\in D}{\text{mín}} f(\x) \\
